...
var Queue = (function() {
  ...
  return class extends require('events') {
     ...
     * @constructor
     * @param head {Promise}
     * @param options - skip errors if truthy
     */
    constructor(head, options) {
      super();
      _q = head || Promise.resolve();
      this.skipErrors = options && options.skip;
      this.thrown = false;
    }
...
var qs = new Queue(Promise.resolve(0));
qs.on('drain', () => {
...
/*this looks like too much trouble...*/
      return qs.push({cb: cb, x: n})            // record the queue state to detect drain event
        .then(() => {
          return f(n, write, cb);               // return a promise that can resolve or reject
        })
        .then(                                  // perform follow-up, sync write
